...
object RetrofitClient {
    ...
    private var commentListRetrofit: Retrofit? = null
    ...
    fun getCommentListService(): CommentListService {
        if(commentListRetrofit == null){
            commentListRetrofit = Retrofit.Builder()
                .baseUrl(ReadmeServerService.BASE_URL)
                .client(client)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
        }
        return commentListRetrofit!!.create(CommentListService::class.java)
    }
